...
const Input: React.FC = () => {
  ...
  return (
    <div className='flex flex-col'>
      {(!form || timedOut) && (
        <section className='sticky bottom-0 flex gap-1 md:gap-5 h-fit m-2 md:m-5 items-end'>
          {listening ? (
            ...
            <textarea
              ...
              tabIndex={0}
              spellCheck={false}
              className='flex-1 p-[13px] outline-none overflow-hidden rounded-lg dark:bg-neutral-800 border bg-neutral-50 dark:border-gray-600 border-gray-300 dark:text-white text-black resize-none disabled:hover:cursor-not-allowed'
      ...
      {!timedOut && form && (
        <section className='px-5'>{form && renderForm(JSON.parse(form))}</section>
      )}
    </div>
  );
};
